...
Since we want some interactivity in our app, we define some state-setting function in our stateful `User` component (`toggleLike()`). We then send that function down  to `LikeButton` as we would do normally with props. Finally, we just reference that function in the child component's `clickHandler()`, and _voila_, `LikeButton` is given a way to change the state of its parent.
